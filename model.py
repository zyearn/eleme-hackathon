...
def place_order(cart_id, token):
    sec, milli = map(float, (yield from call_lua_script(lua_time, [])))
    now = (sec-const.REDIS_BASETIME) * 1000 + math.floor(milli / 1000)
    rtn = yield from call_lua_script(lua_place_order, [cart_id, token, now])
    result = {'err': rtn}
    if rtn == 0:
        user_id = yield from get_token_user(token)
        order_id = random_string()
        yield from r.set('user:%s:order' % str(user_id), order_id)
        yield from r.set('order:%s:user' % order_id, '{}'.format(user_id))
        yield from r.hset('order:cart', order_id, '{}'.format(cart_id))
